...
export default function ChatPage() {
  ...
  const fetchMessages = useCallback(async () => {
    try {
      const response = await fetch(`/api/chat?from=${currentUserId}&to=${params.id}`);
      const data = await response.json();
      if (response.ok) {
  ...
  }, [params.id, fetchLawyerDetails, fetchMessages]);
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 mb-4">
          <div className="flex items-center space-x-4">
            <Link href={`/lawyers/${params.id}`} className="text-gray-600 hover:text-gray-900">
            ...
            <div className="flex-1 text-right">
              <p className="text-sm text-gray-600">You ({currentUserId})</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm h-96 overflow-y-auto p-4 mb-4">
          {messages.length === 0 ? (
            <div className="flex items-center justify-center h-full text-gray-500">
              <p>Start a conversation with {user?.firstName}!</p>
            </div>
          ) : (
